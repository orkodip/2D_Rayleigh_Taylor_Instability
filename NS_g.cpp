...
void NS::vel_bc()
{
	for(int i=1;i<=I;i++)	//free slip and no penetration
	{
	...
	for(int j=1;j<=J;j++)	//periodic condition
	{
		u[j][0]=u[j][I];
		v[j][0]=v[j][I];
	...
	u[0][0]=u[1][0]; u[0][I+1]=u[1][I+1]; u[J+1][0]=u[J][0]; u[J+1][I+1]=u[J][I+1];	//corner points
	v[0][0]=-v[1][0]; v[0][I+1]=-v[1][I+1]; v[J+1][0]=-v[J][0]; v[J+1][I+1]=-v[J][I+1];
}
void NS::P_bc()
{
	for(int j=1;j<=J;j++)	//periodic condition for left and right boundary
	{
		P[j][0]=P[j][I];
		P[j][I+1]=P[j][1];
	}
	for(int i=1;i<=I;i++)	//Neumann for bottom and top boundary
	{
		P[0][i]=P[1][i];
		P[J+1][i]=P[J][i];
	}
...
void NS::calc_fc()
{
	...
	for(int j=1;j<=J;j++)	//left and right periodic boundaries
	{
		rho_fp=2.0/(rho_np1[j][1]+rho_np1[j][0]);
		n.x=(Phi[j][1]-Phi[j][0])/dx;
		n.y=0.25*(Phi[j+1][0]+Phi[j+1][1]-Phi[j-1][0]-Phi[j-1][1])/dy;
		n=n.unit();
		y_eff=CY[j]-0.5*(Phi[j][0]+Phi[j][1])*n.y;
		u_EW[j][0]=u_EW[j][I]=0.5*(us[j][1]+us[j][I])+rdx*rho_fp*(sigma*K_EW[j][0]+(rho_0-rho_1)*Grav.y*y_eff)*(F[j][1]-F[j][0]);
	}
}
void NS::Press()
{
	...
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
			...
			b0[(j-1)*I+i-1]=-rdx2*((u_EW[j][i]-u_EW[j][i-1])/dx+(v_NS[j][i]-v_NS[j-1][i])/dy);
...
void NS::update()
{
	...
	for(int j=1;j<=J;j++)	//left boundary cells
		u_EW[j][0]=u_EW[j][I];
}
void NS::continuity()
{
