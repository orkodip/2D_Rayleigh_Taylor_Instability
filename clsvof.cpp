...
//LEFT AND RIGHT BOUNDARY - PERIODIC CONDITION
//BOTTOM AND TOP BOUNDARY - FREE SLIP AND NO PENETRATION
class CLSVOF:public MBASE
{
	int **tag;	//interface cell tag
	int **LS_tag;	//cells having fixed LS
	VECTOR **XT;	//departure points
	double **F_act,**F1,**F2;	//Actual and intermediate Volume Fractions
	double **Phi_act,**Phi1,**Phi2;	//Actual and intermediate level set functions
	...
	double CUBISTA(double D,double C,double U);	//CUBISTA NVD limitter
	...
	public:
			CLSVOF(); ~CLSVOF();
			void ini(double h,double a,double b);
...
{
	for(int j=1;j<=J;j++)	//left and right ghost nodes (Periodic bc)
	{
		Phia[j][0]=Phia[j][I];
		Phia[j][I+1]=Phia[j][1];
	}
...
void CLSVOF::cell_tag()
{
	...
	for(int j=1;j<=J;j++)	//tagging algorithm
	{
		for(int i=1;i<=I;i++)
		{
			...
			{
				if(i==1) { im1=I; ip1=i+1; }	//periodic indices
				else if(i==I) { im1=i-1; ip1=1; }
				else { im1=i-1; ip1=i+1; }
				tag[j][i]=1;	//tag interfacial cell
				tag[j][im1]=1; tag[j][ip1]=1;	//tag neighbouring cells
				tag[j-1][i]=1; tag[j+1][i]=1;
				tag[j-1][im1]=1; tag[j-1][ip1]=1;
				tag[j+1][im1]=1; tag[j+1][ip1]=1;
			}
		}
	}
...
double CLSVOF::CUBISTA(double D,double C,double U)
{
	double Phic;
	Phic=D-U;
	if(abs(Phic)<=EPS) return C;
	else
	{
		Phic=(C-U)/Phic;
		if((Phic>0.0)&&(Phic<0.375)) Phic=1.75*Phic;
		else if((Phic>=0.375)&&(Phic<=0.75)) Phic=0.75*Phic+0.375;
		else if((Phic>0.75)&&(Phic<1.0)) Phic=0.25*Phic+0.75;
		return (Phic*(D-U)+U);
	}
}
double CLSVOF::U_face(int flag_xy,int flag_f,int i,int j,double **Phia,double V)	//CUBISTA + FOU schemes
{
	if(abs(V)<=EPS) return 0.0;
	double D,C,U;
	int p1,p2,m1,m2;	//periodic counter
	if(flag_xy==0)	//interpolation in X direction
	{
		if(flag_f==0)	//west face
		{
			m1=((i-1)==0) ? I : i-1;
			m2=((i-2)==-1) ? I-1 : (((i-2)==0) ? I : i-2);
			p1=((i+1)==(I+1)) ? 1 : i+1;
			if(V>0.0) { D=Phia[j][i]; C=Phia[j][m1]; U=Phia[j][m2]; }
			else if(V<0.0) { D=Phia[j][m1]; C=Phia[j][i]; U=Phia[j][p1]; }
			return CUBISTA(D,C,U);
		}
		else if(flag_f==1)	//east face
		{
			m1=((i-1)==0) ? I : i-1;
			p1=((i+1)==(I+1)) ? 1 : i+1;
			p2=((i+2)==(I+2)) ? 2 : (((i+2)==(I+1)) ? 1 : i+2);
			if(V>0.0) { D=Phia[j][p1]; C=Phia[j][i]; U=Phia[j][m1]; }
			else if(V<0.0) { D=Phia[j][i]; C=Phia[j][p1]; U=Phia[j][p2]; }
			return CUBISTA(D,C,U);
		}
	...
	else if(flag_xy==1)	//interpolation in Y direction
	{
		...
		{
			if((j>2)&&(j<J))	//use CUBISTA scheme
			{
				if(V>0.0) { D=Phia[j][i]; C=Phia[j-1][i]; U=Phia[j-2][i]; }
				else if(V<0.0) { D=Phia[j-1][i]; C=Phia[j][i]; U=Phia[j+1][i]; }
				return CUBISTA(D,C,U);
			}
			else return (0.5*(Phia[j-1][i]*(1+SGN(V))+Phia[j][i]*(1-SGN(V))));	//FOU scheme
		}
		else if(flag_f==1)	//north face
		{
			if((j>1)&&(j<J-1))	//use CUBISTA scheme
			{
				if(V>0.0) { D=Phia[j+1][i]; C=Phia[j][i]; U=Phia[j-1][i]; }
				else if(V<0.0) { D=Phia[j][i]; C=Phia[j+1][i]; U=Phia[j+2][i]; }
				return CUBISTA(D,C,U);
			}
			else return (0.5*(Phia[j][i]*(1+SGN(V))+Phia[j+1][i]*(1-SGN(V))));	//FOU scheme
		}
		else { cout<<"CLSVOF: ERROR IN UPWIND SCHEME!"<<endl; return 0; }
	}
	else { cout<<"CLSVOF: ERROR IN UPWIND SCHEME!"<<endl; return 0; }
...
void CLSVOF::F_adv(int flag,double **Fa,double **A_xa,double **A_ya)
{
	...
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
			...
			u_fx[0]=U_face(0,0,i,j,A_xa,u_EW[j][i-1]);	//advected velocity for X momentum
			u_fx[1]=U_face(0,1,i,j,A_xa,u_EW[j][i]);
			if(j==1) u_fy[0]=u[1][i];	//bottom boundary (free slip)
			else u_fy[0]=U_face(1,0,i,j,A_xa,v_NS[j-1][i]);
			if(j==J) u_fy[1]=u[J][i];	//top boundary (free slip)
			else u_fy[1]=U_face(1,1,i,j,A_xa,v_NS[j][i]);

			v_fx[0]=U_face(0,0,i,j,A_ya,u_EW[j][i-1]);	//advected velocity for Y momentum
			v_fx[1]=U_face(0,1,i,j,A_ya,u_EW[j][i]);
			if(j==1) v_fy[0]=0.0;	//bottom boundary (no penetration)
			else v_fy[0]=U_face(1,0,i,j,A_ya,v_NS[j-1][i]);
			if(j==J) v_fy[1]=0.0;	//top boundary (no penetration)
			...
			{
				iup=i-1;
				if(iup==0) iup=I;	//left boundary (periodic bc)
				flag_f=1;
			}
			else {iup=i; flag_f=0;}
			...
			{
				iup=i+1;
				if(iup==(I+1)) iup=1;	//right boundary (periodic bc)
				flag_f=0;
			}
...
void CLSVOF::reinit(double **Phia,double **Fa)
{
	...
	{
		Phia[j][0]=Phia[j][I]; LS_tag[j][0]=LS_tag[j][I];	//periodic conditions
		Phia[j][I+1]=Phia[j][1]; LS_tag[j][I+1]=LS_tag[j][1];
	}
	for(int i=0;i<=I+1;i++)	//initialize level sets of the ghost cells(bottom and top boundaries)
	{
		Phia[0][i]=Phia[1][i]; LS_tag[0][i]=LS_tag[1][i];	//Neumann conditions
...
double CLSVOF::adv_err()
{
	...
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
			num+=abs(F[j][i]-F_act[j][i]);
			din+=F_act[j][i];
		}
	}
	num=(Xm[1]-Xm[0])*(Ym[1]-Ym[0])*num;
	//return(num/din);
	return num;
}
