//INITIALIZATION OF THE VOLUME FRACTIONS AND LEVEL SETS FOR A SINUSOIDAL WAVE USING THINC METHOD
class INI	//initial volume fraction and level sets
{
	double *X,*Y,*CX,*CY,**Fa,**Phia;
	double dx,dy;	//grid size
	double beta,H,A,B;	//interface thickness parameter, wave parameters
	double om[3],eta[3];	//Gaussian weights and points
	public:
		INI(double *x,double *y,double *cx,double *cy,double **fa,double **phia,double Beta,double h,double a,double b);	//initialization of the input variables
		void inter();	//initial interface is defined here
		void VF();	//determine the initial volume fractions
		void LS();	//determine the initial level sets
		void reinit(double **Phi,int **tag);	//determine the initial level set function from volume fractions
};
INI::INI(double *x,double *y,double *cx,double *cy,double **fa,double **phia,double Beta,double h,double a,double b)
{
	X=x; Y=y; CX=cx; CY=cy;
...
void INI::inter()
{
	...
	double x[nos+1],pts[nos+1],dx;
	dx=WIDTH/nos;
	x[0]=0.0; pts[0]=H+A;	//initial points of the wave
	for(int i=1;i<=nos;i++)	//generate cosine wave
	{
		x[i]=x[i-1]+dx;
		pts[i]=H+A*cos(B*x[i]);	//Y co-ordinates
	}
	...
	for(int i=0;i<=nos;i++)	//print X coordinates
		p_out<<" "<<x[i];
	p_out<<endl<<endl;
...
//--------------------INITIALIZATION SCHEME (TRACING ALGORITHM)--------------------------------------------------
	for(int i=1;i<=I;i++)	//tracing in vertical direction
	{
		...
		{
		//-----------------VALUE OF RAY---------------------------------------------------
			if(Phi[j][i]<0.0) ray[1]=-1;
			else if(Phi[j][i]>0.0) ray[1]=1;
		//-------------CELL TAGGING AND ADVANCE RAY---------------------------------------
			if((j!=1)&&(j!=J))	//for inner domain only
			{
				if((ray[0]*ray[1])==-1) { tag[j-1][i]=1; tag[j][i]=1; }
			}
			ray[0]=ray[1];	//advance the ray for the next cell
		}
	}
	for(int j=1;j<=J;j++)	//initialize level sets of inner domain
		for(int i=1;i<=I;i++)
			if(tag[j][i]==0)
				Phi[j][i]*=100.0;
	for(int j=1;j<=J;j++)	//initialize level sets of the ghost cells(left and right wall boundaries)
	{
		Phi[j][0]=Phi[j][1]; tag[j][0]=tag[j][1];
		Phi[j][I+1]=Phi[j][I]; tag[j][I+1]=tag[j][I];
	}
	for(int i=1;i<=I;i++)	//initialize level sets of the ghost cells(bottom and top wall boundaries)
	{
		Phi[0][i]=Phi[1][i]; tag[0][i]=tag[1][i];
		Phi[J+1][i]=Phi[J][i]; tag[J+1][i]=tag[J][i];
	}
	Phi[0][0]=Phi[0][1]; tag[0][0]=tag[0][1];	//corner points
	Phi[0][I+1]=Phi[0][I]; tag[0][I+1]=tag[0][I];
	Phi[J+1][0]=Phi[J+1][1]; tag[J+1][0]=tag[J+1][1];
	Phi[J+1][I+1]=Phi[J+1][I]; tag[J+1][I+1]=tag[J+1][I];
